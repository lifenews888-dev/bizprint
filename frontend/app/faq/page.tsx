...
const DEFAULT_HERO = { hero_title: 'Түгээмэл асуулт', hero_desc: 'Хариулт олдохгүй бол бидэнтэй холбогдоорой' }
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '48px 20px', fontFamily: "'DM Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        <h1 style={{ fontSize: 32, fontWeight: 800, color: 'var(--text)', marginBottom: 8 }}>
          {hero.hero_title.includes(' ')
            ? <>{hero.hero_title.split(' ').slice(0, -1).join(' ')} <span style={{ color: '#FF6B00' }}>{hero.hero_title.split(' ').pop()}</span></>
            : <span style={{ color: '#FF6B00' }}>{hero.hero_title}</span>
          }
        </h1>
        <p style={{ fontSize: 14, color: 'var(--text3)' }}>{hero.hero_desc}</p>
      </div>
