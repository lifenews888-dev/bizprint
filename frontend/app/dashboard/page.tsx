    ...
    <div style={{ display:'flex', minHeight:'100vh', background:'#FAFAF8', fontFamily:"'Newsreader','Georgia',serif", color:'#1C1917' }}>
      ...
      {showProposal && (<div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', backdropFilter:'blur(4px)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }} onClick={()=>setShowProposal(false)}><div style={{ background:'#fff', borderRadius:20, width:'100%', maxWidth:720, maxHeight:'90vh', overflow:'auto', boxShadow:'0 24px 80px rgba(0,0,0,0.2)' }} onClick={e=>e.stopPropagation()}>
        ...
        <div id="ppdf" style={{ padding:'32px 36px', fontFamily:"'DM Sans',sans-serif" }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:28, paddingBottom:16, borderBottom:'2px solid #FF6B00' }}><div><div style={{ fontSize:28, fontWeight:700 }}><span style={{ color:'#FF6B00' }}>Biz</span>Print</div><div style={{ fontSize:12, color:'#78716C' }}>BizPrint Platform</div></div><div style={{ textAlign:'right', fontSize:12, color:'#78716C' }}><div>Ogнoo: {new Date().toLocaleDateString()}</div><div>Proposal #{String(Date.now()).slice(-6)}</div></div></div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:24 }}><div style={{ background:'#FAFAF8', borderRadius:10, padding:'14px 18px' }}><div style={{ fontSize:10, fontWeight:600, color:'#A8A29E', textTransform:'uppercase', marginBottom:6 }}>Client</div><div style={{ fontSize:14, fontWeight:600 }}>{user?.full_name}</div><div style={{ fontSize:12, color:'#78716C' }}>{user?.email}</div></div><div style={{ background:'#FAFAF8', borderRadius:10, padding:'14px 18px' }}><div style={{ fontSize:10, fontWeight:600, color:'#A8A29E', textTransform:'uppercase', marginBottom:6 }}>Info</div><div style={{ fontSize:12, color:'#78716C' }}>Items: {selectedQuotesList.length}</div><div style={{ fontSize:12, color:'#78716C' }}>Valid: 3 days</div></div></div>
          <table style={{ width:'100%', borderCollapse:'collapse', marginBottom:20 }}><thead><tr>{['#','Product','Qty','Specs','Unit','Total'].map(h=><th key={h} style={{ padding:'8px 12px', textAlign:h==='Total'||h==='Unit'?'right':'left', fontSize:11, fontWeight:600, color:'#A8A29E', borderBottom:'2px solid #E7E5E4' }}>{h}</th>)}</tr></thead><tbody>{selectedQuotesList.map((q,i)=><tr key={q.id} style={{ borderBottom:'1px solid #F5F5F4' }}><td style={{ padding:'10px 12px', fontSize:12, color:'#78716C' }}>{i+1}</td><td style={{ padding:'10px 12px' }}><div style={{ fontWeight:600, fontSize:13 }}>{q.product_name||'Item'}</div></td><td style={{ padding:'10px 12px', fontSize:12 }}>{q.quantity||0}</td><td style={{ padding:'10px 12px', fontSize:11, color:'#78716C' }}>{[q.paper_gsm?q.paper_gsm+'gsm':null,q.color_mode].filter(Boolean).join(', ')||'-'}</td><td style={{ padding:'10px 12px', textAlign:'right', fontSize:12 }}>{q.unit_price?Number(q.unit_price).toLocaleString()+'T':'-'}</td><td style={{ padding:'10px 12px', textAlign:'right', fontSize:14, fontWeight:700, color:'#FF6B00' }}>{Number(q.total_price||0).toLocaleString()}T</td></tr>)}</tbody></table>
          <div style={{ display:'flex', justifyContent:'flex-end' }}><div style={{ width:260 }}><div style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', fontSize:13, borderBottom:'1px solid #F5F5F4' }}><span style={{ color:'#78716C' }}>Subtotal</span><span style={{ fontWeight:600 }}>{selTotal.toLocaleString()}T</span></div><div style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', fontSize:13, borderBottom:'1px solid #F5F5F4' }}><span style={{ color:'#78716C' }}>VAT 10%</span><span style={{ fontWeight:600 }}>{Math.round(selTotal*0.1).toLocaleString()}T</span></div><div style={{ display:'flex', justifyContent:'space-between', padding:'10px 0', fontSize:16 }}><span style={{ fontWeight:700 }}>Total</span><span style={{ fontWeight:700, color:'#FF6B00', fontSize:20 }}>{Math.round(selTotal*1.1).toLocaleString()}T</span></div></div></div>
