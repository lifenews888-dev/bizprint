      ...
      <div style={{ width:224, minWidth:224, height:'100vh', background:'var(--surface)', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', position:'sticky', top:0, zIndex:20, fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
        ...
        <div style={{ flex:1, padding:'6px', overflowY:'auto' }}>
          {dashMode === 'customer' ? (<>
            ...
            {[{label:'Шинэ захиалга',icon:'➕',href:'/order'},{label:'Үнэ тооцоолох',icon:'🧮',href:'/quote'},{label:'🤖 AI Smart Quote',icon:'🤖',href:'/smart-quote'},{label:'Дэлгүүр',icon:'🛍️',href:'/shop'},{label:'Контент захиалах',icon:'🎨',href:'/dashboard/customer/ugc'}].map(a=>(<button key={a.label} onClick={()=>router.push(a.href)} style={{ width:'100%', display:'flex', alignItems:'center', gap:9, padding:'8px 10px', borderRadius:8, border:'none', background:'transparent', color:'var(--text3)', cursor:'pointer', fontSize:13, fontFamily:"'Segoe UI',system-ui,sans-serif", marginBottom:1 }}><span style={{ fontSize:14, width:15 }}>{a.icon}</span><span>{a.label}</span></button>))}
          </>) : (<>
