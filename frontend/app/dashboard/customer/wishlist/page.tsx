...
export default function WishlistPage() {
  ...
  useEffect(() => {
    ...
    apiFetch<any>('/products', { auth: false }).then(all => {
      if (Array.isArray(all)) setProducts(all.filter((p: any) => wishlist.includes(p.id)))
  ...
  if (products.length === 0) return (
    <div className="min-h-[50vh] flex flex-col items-center justify-center px-4">
      ...
      <a href="/shop" className="inline-flex items-center gap-2 px-6 py-3 bg-[#FF6B00] text-white rounded-xl text-sm font-bold no-underline hover:bg-[#E55D00] transition-colors">
          ...
          {products.map(p => {
            ...
            return (
              <motion.div key={p.id} layout exit={{ opacity: 0, scale: 0.8 }} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] overflow-hidden group">
                <a href={`/product/${p.slug || p.id}`} className="block no-underline">
                ...
                <div className="p-3">
                  <a href={`/product/${p.slug || p.id}`} className="text-sm font-semibold text-[var(--text)] no-underline hover:text-[#FF6B00] transition-colors line-clamp-2">{p.name_mn || p.name}</a>
