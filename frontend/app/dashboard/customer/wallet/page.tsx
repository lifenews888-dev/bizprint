...
export default function WalletPage() {
  ...
  async function submitWithdraw() {
    ...
    if (!amount || amount < 10000) {
      setWithdrawError('Хамгийн бага татах дүн: 10,000₮')
      return
    }
    if (balance && amount > balance.balance) {
      setWithdrawError('Балансаас их дүн оруулах боломжгүй')
      return
    }
    setWithdrawing(true)
    try {
      ...
      setWithdrawError('Алдаа гарлаа. Дахин оролдоно уу.')
    }
  ...
  const roleConfig = ROLE_CONFIG[user?.role || 'customer'] || ROLE_CONFIG.customer
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: 28, marginBottom: 6 }}>{roleConfig.icon}</div>
            ...
            <div style={{
              ...
              borderRadius: 16, padding: '20px 28px', textAlign: 'right',
              ...
              <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-0.5px' }}>
                ...
                <span style={{ fontSize: 16, fontWeight: 400, marginLeft: 4 }}>₮</span>
              </div>
              <button
                onClick={() => setShowWithdraw(true)}
                disabled={!balance?.balance || balance.balance < 10000}
                style={{
                  marginTop: 14,
                  background: balance?.balance && balance.balance >= 10000
                    ? 'rgba(255,255,255,0.95)'
                    : 'rgba(255,255,255,0.3)',
                  color: balance?.balance && balance.balance >= 10000
                    ? roleConfig.color
                    : 'rgba(255,255,255,0.6)',
                  border: 'none', borderRadius: 8,
                  padding: '9px 20px', fontWeight: 700,
                  fontSize: 13, cursor: balance?.balance && balance.balance >= 10000 ? 'pointer' : 'not-allowed',
                }}
              >
                ↑ Татах хүсэлт
              </button>
          ...
          <div style={{ display: 'flex', gap: 24, marginTop: 28 }}>
            <MiniStat label={roleConfig.earnLabel} value={balance?.total_earned || 0} />
            <MiniStat label="Нийт татсан" value={balance?.total_withdrawn || 0} />
            {pendingWithdraws.length > 0 && (
              <div style={{
                ...
                <div style={{ fontSize: 18, fontWeight: 700 }}>
                  {pendingWithdraws.length} хүсэлт
                </div>
              </div>
            )}
          </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 48px' }}>
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontWeight: 600, fontSize: 14, color: '#F59E0B' }}>
                {pendingWithdraws.length} татах хүсэлт хүлээгдэж байна
              </div>
              <div style={{ fontSize: 12, color: 'var(--text2)' }}>
                Admin батлах хүртэл 1-2 ажлын өдөр хүлээнэ үү
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '1px solid var(--border)' }}>
          {([
            { key: 'all',      label: '📋 Бүх гүйлгээ' },
            { key: 'credit',   label: '↑ Орлого'       },
            { key: 'withdraw', label: '↓ Татах'         },
          ] as { key: typeof tab; label: string }[]).map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              background: 'none', border: 'none', padding: '10px 16px',
              fontSize: 13, fontWeight: 600, cursor: 'pointer',
              color: tab === t.key ? roleConfig.color : 'var(--text2)',
              borderBottom: tab === t.key ? `2px solid ${roleConfig.color}` : '2px solid transparent',
              marginBottom: -1,
            }}>
              {t.label}
            </button>
          ))}
          <span style={{ marginLeft: 'auto', alignSelf: 'center', fontSize: 13, color: 'var(--text2)' }}>
        ...
        {filtered.length === 0 ? (
          <div style={{
            ...
            <div style={{ fontWeight: 600, marginBottom: 6 }}>Гүйлгээ байхгүй</div>
            <div style={{ fontSize: 13, color: 'var(--text2)' }}>
              Орлого орж ирэхэд энд харагдана
            </div>
          </div>
        ) : (
          <div style={{
            ...
            {/* Table header */}
            <div style={{
              display: 'grid', gridTemplateColumns: '1fr 120px 140px 140px 120px',
              ...
              <span style={{ textAlign: 'right' }}>Огноо</span>
            </div>

            {filtered.map((tx, i) => {
              const txType  = TX_TYPE_LABEL[tx.type]  || { label: tx.type,  color: 'var(--text2)' }
              const isCredit = tx.type === 'credit' || tx.type === 'commission' || tx.type === 'payment'
              const statusInfo = tx.status ? STATUS_LABEL[tx.status] : null
              ...
              return (
                <div key={tx.id} style={{
                  ...
                  <div>
                    ...
                    {tx.note && (
                      <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 2 }}>{tx.note}</div>
                    )}
                    {statusInfo && (
                      <span style={{
                  ...
                  {/* Type badge */}
                  <span style={{
                    background: `${txType.color}18`, color: txType.color,
                  ...
                  {/* Balance after */}
                  <div style={{ textAlign: 'right', color: 'var(--text2)', fontSize: 13 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <button onClick={() => { setShowWithdraw(false); setWithdrawError('') }} style={{
            ...
            <div style={{
              ...
              <span style={{ fontSize: 18, fontWeight: 700, color: roleConfig.color }}>
                {(balance?.balance || 0).toLocaleString()}₮
              </span>
            </div>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{
                    ...
                    background: 'var(--surface2)', border: `1px solid ${withdrawError ? '#EF4444' : 'var(--border)'}`,
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                {[50000, 100000, 200000, 500000].filter(v => v <= (balance?.balance || 0)).map(v => (
                  <button key={v}
                    ...
                    style={{
                      background: withdrawAmount === String(v) ? roleConfig.color : 'var(--surface2)',
                      color: withdrawAmount === String(v) ? '#fff' : 'var(--text2)',
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input
                ...
                placeholder="Банкны дансны мэдээлэл гэх мэт..."
                style={{
            ...
            <div style={{
              ...
              🔒 Хүсэлт илгээгдсэний дараа admin баталгаажуулна.
              Батлагдсаны дараа л балансаас хасагдана.
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                ...
                style={{
                  flex: 2, padding: '12px',
                  background: withdrawing ? 'var(--border)' : roleConfig.color,
