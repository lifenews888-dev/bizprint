...
const CATEGORIES = ['Бүгд', 'Нэрийн хуудас', 'Флаер', 'Баннер', 'Хаяг', 'Ном', 'Хайрцаг']

const GALLERY = [
  { id: 1, title: 'Корпорэйт нэрийн хуудас', category: 'Нэрийн хуудас', image: 'https://images.unsplash.com/photo-1611532736597-de2d4265fba3?w=600', description: '350gsm, soft-touch ламинат' },
  ...
  { id: 5, title: 'Ресторан флаер', category: 'Флаер', image: 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=600', description: 'A5, 2 талдаа хэвлэл' },
  ...
  { id: 9, title: 'Роллап стенд', category: 'Баннер', image: 'https://images.unsplash.com/photo-1558618666-fcd25c85f82e?w=600', description: '85×200cm, PVC хэвлэл' },
  ...
  { id: 13, title: '3D товгор үсэг', category: 'Хаяг', image: 'https://images.unsplash.com/photo-1563986768609-322da13575f3?w=600', description: 'Нерж, LED гэрэлтэй' },
  ...
  { id: 17, title: 'Компанийн каталог', category: 'Ном', image: 'https://images.unsplash.com/photo-1586075010923-2dd4570fb338?w=600', description: 'A4, 32 хуудас, садлаар' },
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 20px' }}>
      ...
      <div style={{ columnCount: 3, columnGap: 16 }} className="gallery-masonry">
        {filtered.map(g => (
          <div key={g.id} onClick={() => setLightbox(g)} style={{ breakInside: 'avoid', marginBottom: 16, borderRadius: 12, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer', transition: 'box-shadow 0.2s' }}>
            <img src={g.image} alt={g.title} loading="lazy" style={{ width: '100%', display: 'block' }} />
      ...
      {/* Lightbox */}
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ maxWidth: 700, width: '100%', borderRadius: 16, overflow: 'hidden', background: 'var(--surface)' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 4 }}>{lightbox.category}</p>
              <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 16 }}>{lightbox.description}</p>
