...
export default function RegisterPage() {
  ...
  async function register() {
    if (!fullName.trim()) { setError('Бүтэн нэрээ оруулна уу'); return }
    if (!email.trim()) { setError('Имэйл оруулна уу'); return }
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', background: '#111', borderLeft: '1px solid #1A1A1A' }}>
        <div style={{ width: '100%', maxWidth: '380px' }}>
          ...
          <div style={{ marginBottom: '28px' }}>
            <h2 style={{ fontSize: '26px', fontWeight: 700, color: '#F1F5F9', margin: '0 0 6px', letterSpacing: '-0.5px' }}>Бүртгүүлэх</h2>
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            <input type="text" value={fullName} onChange={e => setFullName(e.target.value)}
              placeholder="Овог нэр" style={inp}
          ...
          <div style={{ marginBottom: '14px' }}>
            <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '8px', fontWeight: 500 }}>Утасны дугаар</label>
            <input type="tel" value={phone} onChange={e => setPhone(e.target.value)}
              placeholder="99112233" style={inp}
              onFocus={e => (e.target.style.borderColor = 'var(--orange)')}
              onBlur={e => (e.target.style.borderColor = '#2A2A2A')}
              onKeyDown={e => e.key === 'Enter' && register()} />
          </div>
          ...
          <div style={{ marginBottom: '14px' }}>
            <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '8px', fontWeight: 500 }}>Нууц үг * <span style={{ color: '#444', fontWeight: 400 }}>(хамгийн багадаа 8 тэмдэгт)</span></label>
          ...
          <p style={{ marginTop: '16px', fontSize: '11px', color: '#333', textAlign: 'center', lineHeight: 1.6 }}>
