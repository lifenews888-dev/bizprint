...
export default function DesignerRequestsPage() {
  ...
  async function loadJobs() {
    setLoading(true)
    try {
      const data = await apiFetch<any>(`/design-requests`)
  ...
  async function requestJob(id: string) {
    try {
      await apiFetch<any>(`/design-requests/${id}/assign`, { method: 'PATCH'})
  ...
  async function uploadFile(jobId: string, file: File) {
    ...
    fd.append('file', file)
    try {
      await apiFetch<any>(`/design-requests/${jobId}/upload`, { method: 'POST', body: fd })
  ...
  async function submitForReview(id: string) {
    try {
      await apiFetch<any>(`/design-requests/${id}/submit`, { method: 'PATCH'})
