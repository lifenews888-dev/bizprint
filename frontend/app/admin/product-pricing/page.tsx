  ...
  if (editing) {
    return (
      <div style={{ padding: 24, fontFamily: F, maxWidth: 600, margin: '0 auto' }}>
        ...
        <button onClick={() => setEditing(null)} style={{ background: 'none', border: 'none', color: O, fontSize: 14, cursor: 'pointer', fontFamily: F, marginBottom: 8 }}>&larr; Буцах</button>
    ...
    <div style={{ padding: 24, fontFamily: F }}>
      ...
      {items.length === 0 && (
        <div style={{ textAlign: 'center', padding: 60, color: '#9CA3AF' }}>
          ...
          <p style={{ fontSize: 15, margin: 0 }}>Бүтээгдэхүүний үнэ бүртгэгдээгүй байна</p>
          <p style={{ fontSize: 13, margin: '8px 0 0' }}>+ Шинэ үнэ товчийг дарж эхлээрэй</p>
        </div>
      )}
              ...
              {typeItems.map((item: any) => (
                <div key={item.id} style={{ background: 'var(--surface, #fff)', borderRadius: 14, padding: 20, border: '1px solid var(--border, #E5E7EB)', opacity: item.is_active ? 1 : 0.5 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                    ...
                    <button onClick={() => handleDelete(item.id)} style={{ padding: '7px 12px', background: '#FEE2E2', color: '#DC2626', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: F }}>
                      Устгах
                    </button>
                  </div>
                </div>
