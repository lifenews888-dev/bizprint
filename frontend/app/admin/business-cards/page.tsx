            ...
            <div style={{ marginTop: 16, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
              ...
              {(l.backgrounds || []).filter((bg: any) => bg.side === edSide).map((bg: any) => (
                <div key={bg.id} style={{ position: 'relative', marginTop: 6, borderRadius: 6, overflow: 'hidden', border: '1px solid var(--border)' }}>
                  <img src={bg.url?.startsWith('http') ? bg.url : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}${bg.url}`} alt="" style={{ width: '100%', height: 60, objectFit: 'cover' }} />
                  <button onClick={async () => {
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              ...
              >
                ...
                {(() => {
                  ...
                  return bgImg ? <img src={bgImg.url?.startsWith('http') ? bgImg.url : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}${bgImg.url}`} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} /> : null
                })()}
