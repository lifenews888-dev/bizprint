...
export default function Footer() {
  ...
  const siteName = (settings.site_name || 'BizPrint') as string
  const phone = (settings.site_phone || '+976 7711-7700') as string
  const copyright = (settings.footer_copyright || '© 2026 BizPrint. Бүх эрх хуулиар хамгаалагдсан.') as string
  const columns: any[] = settings.footer_columns || FALLBACK_COLUMNS
  const showSocial = settings.footer_show_social !== false

  const SOCIAL_PLATFORMS = ['facebook', 'instagram', 'youtube', 'tiktok', 'twitter', 'linkedin', 'pinterest']
  const socialLinks = SOCIAL_PLATFORMS
    .filter(p => settings[`site_${p}`])
    .map(p => ({ platform: p, url: settings[`site_${p}`] as string }))

  const helpCards = [
    { icon: 'help', url: '/faq', title: 'Тусламж & FAQ', cta: 'Харах →' },
    { icon: 'feedback', url: '/contact', title: 'Санал хүсэлт', cta: 'Бичих →' },
    { icon: 'phone', url: `tel:${phone}`, title: 'Утасны дэмжлэг', cta: 'Залгах →' },
  ]

  const HELP_ICONS: Record<string, () => JSX.Element> = {
    help: HelpIcon,
  ...
  const PAYMENT_MAP: Record<string, () => JSX.Element> = {
    visa: VisaIcon,
