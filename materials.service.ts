...
export class MaterialsService {
  ...
  async calcMaterialCost(dto: CalcMaterialCostDto): Promise<MaterialCostResult> {
    ...
    // Цаасны зардал
    const paperCost = paper.pricePerSheet * sheets;

    // Бэхний зардал — m² × coverage × price/liter
    const areaM2 = (dto.widthMm / 1000) * (dto.heightMm / 1000) * sheets;
    const colorChannels = dto.colorMode === 'CMYK' ? 4
      : dto.colorMode === '2C' ? 2
      : dto.colorMode === '1C' ? 1 : 1;
    const mlUsed = areaM2 * ink.coverageRateMlPerM2 * colorChannels;
    const inkCost = (mlUsed / 1000) * ink.pricePerLiter;
  ...
  async seedDefaults(): Promise<void> {
    ...
    if (paperCount === 0) {
      await this.paperRepo.save([
        ...
        { name: 'Banner vinyl 510gsm', size: 'custom', weightGsm: 510, pricePerSheet: 1200, supplier: 'Баннер Моул', stockQty: 500, reorderLevel: 50 },
    ...
    if (inkCount === 0) {
      await this.inkRepo.save([
        { name: 'Стандарт CMYK', type: 'CMYK', coverageRateMlPerM2: 2.5, pricePerLiter: 45000, supplier: 'Моул бэх' },
        { name: 'УВ лак', type: 'UV', coverageRateMlPerM2: 1.8, pricePerLiter: 85000, supplier: 'Моул бэх' },
        { name: 'Дижитал тонер', type: 'digital_toner', coverageRateMlPerM2: 0.8, pricePerLiter: 120000, supplier: 'HP' },
    ...
    if (finCount === 0) {
      await this.finishingRepo.save([
        ...
        { name: 'Зүсэх', type: FinishingType.CUT, setupCost: 2000, unitPrice: 15, timePerUnitMinutes: 0.02 },
